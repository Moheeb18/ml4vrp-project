...
from Solver_V6 import solve_vrp as _v6_solve_vrp
...
def seed_v6_nearest_neighbour(coords, demands, capacity, distance_matrix):
    """
    Solver_V6 greedy nearest-neighbour construction.
    Uses V6's own solve_vrp so the seeding strategy is consistent with
    the local search solver that runs during the GA.  This is a fast,
    deterministic seed that is structurally different from Clark-Wright
    (proximity-driven rather than savings-driven).
    """
    routes = _v6_solve_vrp(coords, demands, capacity, distance_matrix)
    return _routes_to_chrom(routes)


# ---------------------------------------------------------------------------
# NEAREST NEIGHBOUR (numpy-based fallback, kept for Tier 2 seeding)
# ---------------------------------------------------------------------------

def nearest_neighbour_chromosome(coords, demands, capacity):
    """Greedy nearest-neighbour construction (numpy distance, no DM needed)."""
...
# solution via a different savings formula, merge strategy, or construction
# heuristic.  Solver_V6 NN is included alongside the CW variants.
_SEED_VARIANTS = [
    seed_standard,
    ...
    seed_v6_nearest_neighbour,   # V6 NN — different construction principle
]
...
def initialize_population(pop_size, num_customers,
    ...
    Build an initial population with a diverse seeding strategy:
      ...
      Tier 1 — Diverse seeds (40% of population)
        Five Clark-Wright variants + Solver_V6 nearest-neighbour.
        Each produces a structurally different solution:
          • Standard CW, demand-weighted CW, conservative CW,
            reversed-demand CW, distance-penalised CW,
            Solver_V6 proximity NN.
        Each is lightly perturbed (5% random swaps) to generate
        multiple unique chromosomes per variant.

      Tier 2 — Nearest-neighbour seeds (20% of population)
        Fast greedy NN (numpy-based), perturbed for variety.
    ...
    if smart_seeding:
        ...
        for seed_fn in _SEED_VARIANTS:
            ...
            # Also add one noisy CW variant per seed for extra variety
            try:
