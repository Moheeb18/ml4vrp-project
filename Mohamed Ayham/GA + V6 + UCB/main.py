...
GA_CONFIG = {
    ...
    "crossover_rate" : 0.9,
    "mutation_rate"  : 0.03,
    "patience"       : 200,   # ga_core enforces >= 3 x ls_interval automatically
    "ls_elite_size"  : 5,
    "ls_final"       : True,
}
...
def solve_instance(file_path):
    ...
    if num_customers > 200:
        # Large X instances: smaller pop for speed, more gens to compensate,
        # high patience so the GA doesn't quit before LS runs.
        # Mutation rate raised to 0.05 — larger search space needs more
        # diversity pressure. Completely isolated from A/B instances.
        config["pop_size"]        = 100
        config["num_generations"] = 600
        config["patience"]        = 200
        config["mutation_rate"]   = 0.05
    elif num_customers > 80:
        # Medium X instances: moderate pop, plenty of generations
        config["pop_size"]        = 150
        config["num_generations"] = 500
        config["patience"]        = 200
    else:
        # Small instances (A/B): default config is fine
