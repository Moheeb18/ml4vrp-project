...
def run_ga(
    ...
    (competition objective from ML4VRP)

    Key improvements over a plain GA:
      - Nearest neighbour seeding for 20% of initial population
      - RL agent (Q-learning) adaptively controls mutation/crossover rates
      - Local search applied to top elites every 10 generations
      - Full local search pipeline: 2-opt + OR-opt + inter-route relocation
        + inter-route swap + route elimination
      - Final local search pass on best solution before returning

    Returns:
        best_chromosome (list)
        best_routes     (list)
        best_distance   (float): competition objective score
    ...
    # -----------------------------------------------------------------------
    # 2. Initialise population with NN seeding
    # -----------------------------------------------------------------------
    ...
    for gen in range(1, num_generations + 1):
        ...
        # -- Update LS-improved scores in population ------------------------
        ...
        if no_improve >= patience:
            if verbose:
                print(f"\n  Early stopping at gen {gen} "
                      f"(no improvement for {patience} gens).")
            break

    # -----------------------------------------------------------------------
    # 4. Final full local search on best solution
    # -----------------------------------------------------------------------
