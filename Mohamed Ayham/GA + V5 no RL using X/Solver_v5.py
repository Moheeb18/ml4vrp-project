...
def best_merge_of_two_routes(route1, route2, distance_matrix):
    candidates = []

    a = route1[:-1] + route2[1:]
    b = route2[:-1] + route1[1:]
    c = route1[:-1] + list(reversed(route2[1:-1])) + [0]
    d = route2[:-1] + list(reversed(route1[1:-1])) + [0]

    for merged in [a, b, c, d]:
        if merged[0] == 0 and merged[-1] == 0:
            candidates.append(two_opt(merged, distance_matrix))

    best_route = min(candidates, key=lambda r: route_distance(r, distance_matrix))
    return best_route


def try_merge_routes(routes, demands, capacity, distance_matrix, penalty_per_vehicle=1000):
    improved = True
    best_routes = [r[:] for r in routes]

    while improved:
        improved = False
        current_score = solution_score(best_routes, distance_matrix, penalty_per_vehicle)
        ...
        for i in range(len(best_routes)):
            for j in range(i + 1, len(best_routes)):
                ...
                if route_demand(route1, demands) + route_demand(route2, demands) > capacity:
                    continue

                merged_route = best_merge_of_two_routes(route1, route2, distance_matrix)

                if route_demand(merged_route, demands) > capacity:
                    continue

                candidate_routes = [r[:] for idx, r in enumerate(best_routes) if idx not in (i, j)]
                candidate_routes.append(merged_route)

                candidate_score = solution_score(candidate_routes, distance_matrix, penalty_per_vehicle)
...
def improve_routes(routes, demands, capacity, distance_matrix, k=25, lns_iterations=60, remove_ratio=0.20):
    ...
    routes = try_merge_routes(routes, demands, capacity, distance_matrix, penalty_per_vehicle=1000)
